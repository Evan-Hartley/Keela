...
private:
   ...
   * video-direction     : Video direction: rotation and flipping
              flags: readable, writable, controllable, changeable in NULL,
   READY, PAUSED or PLAYING state Enum "GstVideoOrientationMethod" Default: 0,
   "identity" (0): identity         - GST_VIDEO_ORIENTATION_IDENTITY (1): 90r -
   GST_VIDEO_ORIENTATION_90R (2): 180              - GST_VIDEO_ORIENTATION_180
...
public:
  ...
  [[deprecated]]
  void SetBinning(uint32_t binning);
  [[deprecated]]
  uint32_t GetBinning() {
    return binning_factor;
  };
  [[deprecated]]
  uint32_t GetBinningMin() {
    return std::numeric_limits<uint32_t>::min();
  };
  [[deprecated]]
  uint32_t GetBinningMax() {
    return std::numeric_limits<uint32_t>::max();
  };

private:
  Keela::SimpleElement video_scale = SimpleElement("videoscale");
  Keela::SimpleElement scaled_caps_filter =
      SimpleElement("capsfilter", "scaled");
  Keela::SimpleElement native_caps_filter =
      SimpleElement("capsfilter", "native");
