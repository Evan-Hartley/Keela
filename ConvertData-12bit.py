...
# Note: This file is reading 12-bit left shifted data into a 16-bit data slot
# This is the saving process of the data aquired from Keela
...
def extract_luma_3d(path):
    ...
    # run ffmpeg to extract Y plane as 16‑bit grayscale little-endian
    cmd = [
        "ffmpeg",
        "-i", path,
        "-map", "0:v:0",
    ...
    # read raw bytes
    frame_bytes = W * H * 2                         # 2 for uint16
    raw = subprocess.check_output(cmd)

    # convert to array
    arr = np.frombuffer(raw, dtype="<u2")           # 16-bit little-endian
    arr = arr.reshape((T, H, W))

    return arr                                      # uint16, values 0–4095 for 16-bit Y
